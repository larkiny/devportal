...
- **MUST** contain a file `contract.json` representing the contract metadata (as described in [ARC-4](/arc-0004)), and as extended by future potential ARCs).
...
- **MAY** contain the files `approval.teal` and `clear.teal`, that are the compiled versions of approval and clear program in TEAL.
  - Note that `approval.teal` will not be able to contain the application information as this would create circularity. If `approval.teal` is provided, it is assumed that the _actual_ `approval.teal` that is deployed corresponds to `approval.teal` with the proper `bytecblock` (defined below) appended at the end.
...
Block explorers and wallets for _private_ networks **MAY** allow uploading the application information as a .zip or .tar.gz.
...
The encoded information byte string is included in the _approval program_ of the application via a <a href="https://developer.algorand.org/docs/get-details/dapps/avm/teal/opcodes/#bytecblock-bytes">`bytecblock`</a> with a unique byte string equal to the encoding information byte string.
...
- example of the contract metadata that follow [ARC-4](/arc-0004) [contract.json](https://raw.githubusercontent.com/algorandfoundation/ARCs/main/assets/arc-0023/application_information/contract.json).
